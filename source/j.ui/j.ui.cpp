...
void ui_paint(t_ui *x, t_object *view)
{
	...
	if (x->has_gain) {
		...
		jgraphics_arc(g, right_side+6.5, 3.0+6.5, 6.5, PI / 2, (PI/2) + TWOPI * (1.0 + (gain - 12.0) / 108.0)); // angles are in radians
		...
		jgraphics_arc(g, right_side+6.5, 3.0+6.5, 6.5, PI / 2, (PI/2) + TWOPI * (1.0 + (gain - 12.0) / 108.0));
		jgraphics_line_to(g, right_side+6.5, 3.0+6.5);
		jgraphics_stroke(g);
...
void ui_mousedragdelta(t_ui *x, t_object *patcherview, t_pt pt, long modifiers)
{
	...
	if (x->mixDragging) {
		...
		x->anchorValue = x->anchorValue - (pt.y * factor);
		TTLimit(x->anchorValue, -96.0f, 12.0f);
