...
void model_preset_doread_again(TTPtr self)
{
	...
	if (!tterr) {
		...
		critical_enter(0);
		tterr = aTextHandler->sendMessage(kTTSym_ReadAgain);
		critical_exit(0);
...
void model_preset_dowrite(TTPtr self, SymbolPtr msg, AtomCount argc, AtomPtr argv)
{
	...
	if (EXTRA->presetManager) {
		...
		snprintf(filename, MAX_FILENAME_CHARS, "%s.%s.preset", x->patcherClass.c_str(), x->patcherContext.c_str());
...
void model_preset_dowrite_again(TTPtr self)
{
	...
	if (!tterr) {
		...
		critical_enter(0);
		tterr = aTextHandler->sendMessage(kTTSym_WriteAgain);
		critical_exit(0);
